...
# Observaciones: la mutación se aplica a padres que no se cruzan en vez de mutar a los hijos.
import numpy as np
import copy
...
def cruza(pareja: list) -> list:
    ...
    while not all(0 < (funcion_restriccion(individuo) <= 30) for individuo in hijos):
        ...
        # print(f"{hijo_1}: ${funcion_costo(hijo_1)}, {funcion_restriccion(hijo_1)}")
        # print(f"{hijo_2}: ${funcion_costo(hijo_2)}, {funcion_restriccion(hijo_2)}")
...
print('-'*30, "Los resultados son:", '-'*30)
